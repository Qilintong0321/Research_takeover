...
def test_model(model, ep, gmaker,  batch_size):
        ...
        for batch in ep:
            ...
            for b in range(batch_size):
                ...
                ligand_file = os.path.join('/home/qilintong/pdbbind',batch[b].coord_sets[0].src.replace('protein_nowat.gninatypes', 'ligand.pdb'))
                ligand = Chem.MolFromPDBFile(ligand_file)
                ligand = AllChem.GetMorganFingerprintAsBitVect(ligand, nBits=2048, radius=2)
            ...
            print(input_tensor[:,:14].size())
            ligands = torch.stack(ligands)
            print(ligands.size())
            ligands = ligands.to(device =torch.device('cuda'))
            output = model(input_tensor[:,:14],ligands)
